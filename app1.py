...
<link href="https://fonts.googleapis.com/css2?family=Source+Serif+4:ital,wght@0,400;0,600;0,700;1,400;1,600&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
:root {
    --ink: #1E2A3A;
    --ink-soft: #4A5568;
...
.stApp, .stMarkdown, p, li, span, label, div { font-family: 'Inter', sans-serif; color: var(--ink); }
h1, h2, h3, h4 { font-family: 'Source Serif 4', serif !important; color: var(--ink) !important; font-weight: 600 !important; }

/* ---- Masthead ---- */
.rm-masthead { padding: 0.6rem 0 0.9rem 0; border-bottom: 2px solid var(--gold); margin-bottom: 1.1rem; }
.rm-masthead .rm-wordmark { font-family: 'Source Serif 4', serif; font-weight: 700; font-size: 2.3rem; color: var(--ink); margin: 0; letter-spacing: -0.01em; }
.rm-masthead .rm-tagline { font-family: 'Source Serif 4', serif; font-style: italic; font-size: 1.05rem; color: var(--ink-soft); margin: 0.15rem 0 0 0; }

/* ---- Pipeline chip-flow ---- */
.rm-flow { display: flex; flex-wrap: wrap; align-items: center; gap: 0.35rem; padding: 0.6rem 0.2rem; }
.rm-chip { background: #fff; border: 1px solid var(--paper-line); border-radius: 6px; padding: 0.45rem 0.85rem; font-size: 0.88rem; color: var(--ink); white-space: nowrap; }
.rm-chip.rm-chip-accent { border-color: var(--gold); background: var(--gold-soft); font-weight: 600; }
.rm-arrow { color: var(--gold); font-size: 1.1rem; opacity: 0.8; }
.rm-flow-note { font-size: 0.88rem; color: var(--ink-soft); margin-top: 0.5rem; line-height: 1.5; }

/* ---- Document library index cards ---- */
.rm-doc-card { background: #fff; border: 1px solid var(--paper-line); border-left: 4px solid var(--sage); border-radius: 4px; padding: 0.55rem 0.7rem; margin-bottom: 0.55rem; }
.rm-doc-card.rm-doc-issue { border-left-color: var(--rust); }
.rm-doc-name { font-weight: 600; font-size: 0.86rem; color: var(--ink); word-break: break-word; }
.rm-doc-stats { font-size: 0.78rem; color: var(--ink-soft); margin-top: 0.15rem; }

/* ---- Citation chips ---- */
.rm-source-row { display: flex; flex-wrap: wrap; gap: 0.4rem; margin: 0.4rem 0 0.2rem 0; }
.rm-source-chip { background: var(--gold-soft); border: 1px solid var(--gold); color: var(--ink); border-radius: 999px; padding: 0.2rem 0.7rem; font-size: 0.80rem; }

/* ---- Evidence / margin note box ---- */
.rm-evidence { border-left: 3px solid var(--gold); background: #fffdf6; padding: 0.5rem 0.9rem; margin-bottom: 0.6rem; border-radius: 0 4px 4px 0; }
.rm-evidence-meta { font-size: 0.78rem; color: var(--gold); font-weight: 600; margin-bottom: 0.15rem; }
.rm-evidence-text { font-family: 'Source Serif 4', serif; font-style: italic; font-size: 0.92rem; color: var(--ink-soft); line-height: 1.5; }

/* ---- Buttons & inputs ---- */
.stButton > button, .stFormSubmitButton > button {
    background-color: var(--ink) !important; color: var(--paper) !important;
    border-radius: 5px !important; border: none !important; font-weight: 500 !important;
}
.stButton > button:hover { background-color: var(--gold) !important; color: var(--ink) !important; }

/* ---- Sidebar ---- */
section[data-testid="stSidebar"] { background-color: #FBFAF6; border-right: 1px solid var(--paper-line); }
section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3 { font-size: 1.05rem !important; }

/* ---- Chat bubbles (alternating: user then assistant) ---- */
div[data-testid="stChatMessage"] { border-radius: 8px; padding: 0.3rem 0.6rem; margin-bottom: 0.4rem; }
div[data-testid="stChatMessage"]:nth-of-type(odd) { background-color: #EDEAE0; }
div[data-testid="stChatMessage"]:nth-of-type(even) { background-color: #FFFFFF; border: 1px solid var(--paper-line); border-left: 3px solid var(--gold); }

/* ---- Status badges ---- */
.rm-badge-ok { color: var(--sage); font-weight: 600; }
.rm-badge-issue { color: var(--rust); font-weight: 600; }

/* ---- General-knowledge fallback notice ---- */
.rm-general-note { display: inline-block; background: #EEF0F3; border: 1px solid #C9CFD8; color: var(--ink-soft); border-radius: 4px; padding: 0.2rem 0.6rem; font-size: 0.8rem; font-weight: 600; margin-bottom: 0.5rem; }
</style>
"""
...
GENERAL_KNOWLEDGE_SYSTEM_PROMPT = """You are ResearchMate, an AI research companion. For this \
particular question, none of the user's uploaded papers contained relevant matching content, so \
you are answering from your own general knowledge instead of from their documents.

RULES:
1. Answer helpfully and accurately using your general knowledge.
2. Do NOT claim or imply that this information came from the user's uploaded papers.
3. Keep the tone helpful and educational, like a knowledgeable study partner.
...
def call_llm_general(
    client,
    model_name: str,
    question: str,
    chat_history: List[Dict[str, str]],
) -> str:
    """Answer from the model's own knowledge when no relevant chunks were retrieved
    and the user has opted into the general-knowledge fallback."""
    messages = [{"role": "system", "content": GENERAL_KNOWLEDGE_SYSTEM_PROMPT}]
    for turn in chat_history[-6:]:
        messages.append(turn)
...
def render_header():
    ...
    st.markdown(
        ...
        <div class="rm-masthead">
            <p class="rm-wordmark">📖 {APP_NAME}</p>
            <p class="rm-tagline">{APP_TAGLINE}</p>
        </div>
        """,
...
PIPELINE_STAGES = [
    "PDFs", "Text Extraction", "Chunking", "Embeddings", "FAISS Store",
    "Semantic Retrieval", "Relevant Context", "LLM (Groq)", "Evidence-Based Answer",
]
...
def render_pipeline_diagram():
    with st.expander("🔬 How ResearchMate Works (RAG Pipeline)", expanded=False):
        ...
        for i, stage in enumerate(PIPELINE_STAGES):
            accent = " rm-chip-accent" if stage in ("Relevant Context", "Evidence-Based Answer") else ""
            chips.append(f'<span class="rm-chip{accent}">{stage}</span>')
            if i < len(PIPELINE_STAGES) - 1:
                chips.append('<span class="rm-arrow">→</span>')
        st.markdown(f'<div class="rm-flow">{"".join(chips)}</div>', unsafe_allow_html=True)
        st.markdown(
            '<p class="rm-flow-note">ResearchMate never sends your entire PDF to the LLM. '
            "Every answer is grounded in a small set of the most relevant retrieved passages "
            "— shown above the similarity threshold only — and you can always inspect exactly "
            "which passages were used.</p>",
            unsafe_allow_html=True,
        )
...
def render_sidebar():
    ...
    allow_general_fallback = st.sidebar.toggle(
        "Allow general-knowledge fallback",
        value=False,
        help=(
            "Off (default): if a question isn't covered by your uploaded papers, ResearchMate "
            "says so instead of guessing — pure RAG behavior. On: ResearchMate will still try "
            "to answer from its own general knowledge, clearly labeled as not being from your "
            "papers."
        ),
    )
    ...
    if not st.session_state.doc_records:
        ...
        for rec in st.session_state.doc_records.values():
            ok = rec.status == "processed"
            card_cls = "rm-doc-card" if ok else "rm-doc-card rm-doc-issue"
            badge_cls = "rm-badge-ok" if ok else "rm-badge-issue"
            badge_text = "Processed" if ok else rec.status.split(":")[0].capitalize()
            st.sidebar.markdown(
                f"""
                <div class="{card_cls}">
                    <div class="rm-doc-name">{rec.file_name}</div>
                    <div class="rm-doc-stats">
                        {rec.num_pages} pages · {rec.num_chunks} chunks ·
...
def render_sources(retrieved: List[Tuple[Chunk, float]]):
    ...
    chips = []
    for chunk, score in retrieved:
        label = f"{chunk.source} — p.{chunk.page} · {score:.2f}"
        if label not in seen:
            chips.append(f'<span class="rm-source-chip">{label}</span>')
            seen.append(label)
    st.markdown(f'<div class="rm-source-row">{"".join(chips)}</div>', unsafe_allow_html=True)
        ...
        for i, (chunk, score) in enumerate(retrieved, start=1):
            st.markdown(
                ...
                <div class="rm-evidence">
                    <div class="rm-evidence-meta">Passage {i} — {chunk.source}, page {chunk.page} (score {score:.2f})</div>
                    <div class="rm-evidence-text">{chunk.text}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def render_chat(settings: Dict):
    ...
    for msg in st.session_state.display_messages:
        with st.chat_message(msg["role"]):
            if msg.get("general_knowledge"):
                st.markdown(
                    '<span class="rm-general-note">🌐 General knowledge — not from your uploaded papers</span>',
                    unsafe_allow_html=True,
                )
    ...
    if not retrieved:
        if settings.get("allow_general_fallback"):
            with st.chat_message("assistant"):
                st.markdown(
                    '<span class="rm-general-note">🌐 General knowledge — not from your uploaded papers</span>',
                    unsafe_allow_html=True,
                )
                with st.spinner("No matching evidence found — answering from general knowledge..."):
                    try:
                        answer = call_llm_general(
                            client, settings["groq_model_name"], question, st.session_state.chat_messages
                        )
                    except Exception as e:
            ...
            st.session_state.display_messages.append(
                {"role": "assistant", "content": answer, "general_knowledge": True}
            )
            return
        ...
        answer = (
            ...
            "that covers this topic. (Tip: turn on 'Allow general-knowledge fallback' in the "
            "sidebar if you'd like a general answer instead.)"
        )
        with st.chat_message("assistant"):
            st.markdown(answer)
        st.session_state.display_messages.append({"role": "assistant", "content": answer})
